"""
The amp's eye and its descending-neuron readout.
...
descending neurons a amp actually walks with.
"""
import numpy as np
from ampsim import ampBrain


class ampEye:
    """Retinotopic sampling of the screen onto the amp's 892 hex columns."""
    ...
    def look(self, img, cx, cy, fov_w=300, fov_h=210, max_hz=180.0):
        ...
        The amp's gaze follows its own cursor, so the view is egocentric.
        ...
        the amp moves, and there is no positional signal to learn from. A tight
        window makes what the amp sees depend on where it is.
        """
        H, W = img.shape
...
class ampPilot:
    """
    Closed loop: the amp looks at the form, its descending neurons fire, the
    ...
    def __init__(self, fb, eye=None, sim_steps=100, click_hz=330.0):
        ...
        self.motor = {
            "steer_L": dn("DNa02", "L"),   # a amp turns by DNa02 asymmetry
            "steer_R": dn("DNa02", "R"),
            "fwd_L": dn("DNa01", "L"),
            "fwd_R": dn("DNa01", "R"),
    ...
    def step(self, img, cx, cy, gains=None, seed=0, detail=False):
        ...
        # The click is the amp stopping. MN9 was the obvious choice - proboscis
        # extension is the amp's commit action - but MN9 is a taste motor
        ...
        # amp's actual visual field, not a picture of the screen.
        try:
