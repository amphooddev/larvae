...
the pages the amphood encountered, and writes the journal. They talk over the
...
def voice_enabled():
    if os.environ.get("amphood_VOICE_MODEL", "").strip().lower() == "stub":
...
def main():
    ...
    voice_env["amphood_STREAM"] = f"http://127.0.0.1:{PORT}"

    procs = [Proc("roam", [sys.executable, "roam.py"], roam_env)]
    if voice_enabled():
        procs.append(Proc("voice", [sys.executable, "voice.py", "--loop"], voice_env))
    else:
        say("voice disabled: no OPENROUTER_API_KEY and amphood_VOICE_MODEL is not 'stub'")
    ...
    sd = os.environ.get("amphood_STATE_DIR")
    if sd:
        say(f"state dir {sd}: exists={os.path.isdir(sd)} mount={os.path.ismount(sd)}")
        if not os.path.ismount(sd):
            say("WARNING: amphood_STATE_DIR is not a mount point; state will not survive a redeploy")
