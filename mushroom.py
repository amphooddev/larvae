"""
The amp's own learning circuit, driven by what happens on the internet.
...
where a amp's weights actually change: the Kenyon cell to MBON synapse in the
...
synapse depressed - not strengthened. Learning in a amp is subtraction: the
...
* The reward signal is not. A amp is rewarded by sugar, not by reaching a web
  page. Novelty stands in for it here, which is a modelling choice made by a
  person, and the amp has no say in it.
...
# amp_STATE_DIR at it, or every redeploy wipes what the amp has learned.
STORE = Path(os.environ.get("amp_STATE_DIR", str(ROOT / "build"))) / "mb_gains.npz"
