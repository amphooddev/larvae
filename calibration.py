...
The simulator (amphoodsim.py, after Shiu et al. 2024) gives every neuron the same
parameters and every synapse 0.275 mV per contact. Measured on this connectome
(2026-09-15), that stock brain is not a amphood's mushroom body: any odour ignites
...
amphoodsim leaves one free efficacy per cell type (`gains`). The settings below
...
the trained one: 0 is perfectly specific, 1 is fully global.
  ...
  * shock can stay specific (leak ~0.2) when the shocked odour is sparse and
    ...
    twice the synapses
So the honest claim is: sugar and shock change the amphood's response partly to
...
# Cell-type groups a setting may scale, matched against amphoodBrain.type_names.
GROUPS = {
...
# The setting the amphood runs on. The three best tie on learning leak (0.45-0.48);
...
def gains_for(fb, setting):
    """
    A per-type gains vector for amphoodBrain.run, or None for the stock brain.
...
def valence(run_result):
    """Approach minus avoidance, in Hz, from a amphoodBrain.run recorded with readout(mb)."""
