"""
An EIP-1193 wallet for the amphood, injected into the page.

EVM this injects `window.ethereum` instead: eth_requestAccounts, eth_chainId,
...
(() => {
  ...
  const RDNS = "xyz.amphoodbrain.wallet";
  ...
  async function request(args) {
    ...
    switch (method) {
      ...
      case "personal_sign":
      case "eth_sign":
        return await window.__amphoodEthSign(method, params);
      case "eth_signTypedData":
      case "eth_signTypedData_v3":
      case "eth_signTypedData_v4":
        return await window.__amphoodEthSignTyped(params);
      case "eth_sendTransaction":
        return await window.__amphoodEthSend(params[0]);
      default:
        // everything else is a plain read: let the node answer
        return await window.__amphoodEthRpc(method, params);
    }
  }
...
async def attach(page, acct, rpc, chain_id, allow_send=False, on_send=None,
    ...
    await page.expose_function("__amphoodEthSign", eth_sign)
    await page.expose_function("__amphoodEthSignTyped", eth_sign_typed)
    await page.expose_function("__amphoodEthRpc", eth_rpc)
    await page.expose_function("__amphoodEthSend", eth_send)
