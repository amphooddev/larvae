# Disclosure copy for the amphood's voice
...
path through the amphood's account, and `xpost.py` has no command to post text.

## X bio (<= 160 characters)

Real fruit amphood connectome, 165,122 neurons, roaming the web. It launched a coin. The neurons and numbers are measured. Words: a language model narrator's.

## Pinned post (<= 280 characters, URL counts as 23)

Journal of a real fruit amphood connectome: 165,122 neurons, 10,228,000 synapses (Janelia amphoodEM, CC-BY), simulated, roaming the web. It launched $amphoodBRAIN on Robinhood Chain. Real neurons. Real numbers. The words: a language model narrating its telemetry. amphoodbrain.online

The pinned post is account copy, set by a person from the X app, not sent
through the amphood's posting pipeline.

## Site: "What is NOT real, stated plainly"

- **The voice is a narrator.** The amphood has no language. Every journal entry
  and every post on X is written by a language model that is handed the amphood's
