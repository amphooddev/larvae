"""
The amphood, loose on the internet.

A page is screenshotted, sampled through the amphood's 892 retinotopic hex columns
...
- the stopping neuron - clicks. If the click lands on a link, the amphood is
somewhere new. Nothing chooses where it goes. That is the whole point.

Be clear about what this is. A amphood brain has no language, no goals and no plan.
...
* No typing. The amphood has no keyboard at all - it cannot fill a field, write a
...
* A hop budget. When it runs out the amphood is put back on a seed page, so a dead
...
from amphoodsim import amphoodBrain
from envcfg import load_env
from mushroom import MushroomBody
from amphoodeye import amphoodPilot
...
# Link-rich, text-heavy, safe places to be dropped into. The amphood leaves them
...
SEEDS = [
    ...
    # out the amphood is put back on a seed, so if the seeds are a short fixed
...
# allowed, so a click there goes nowhere, the budget expires and the amphood is
...
# the amphood stays inside a set of domains unless someone deliberately opens it.
...
OPEN = load_env().get("amphood_ROAM_OPEN") == "1"
...
BLOB_TOKEN = load_env().get("amphood_BLOB_TOKEN", "")
BLOB_BASE = load_env().get("amphood_BLOB_BASE", "")
...
def blob_put(path, data, ctype):
    ...
    no cleanup to do. Failures are swallowed on purpose: the amphood roaming and
...
def start_tunnel(port):
    ...
    without an account; the address is random and changes every run, so the amphood
...
LIVE_REPO = "fruitamphooddev/amphoodcoinrh"
...
def publish_address():
    ...
    tok = load_env().get("amphood_GH_TOKEN", "")
    url = TUNNEL["url"]
    ...
    try:
        ...
        try:
            ...
            pass
        payload = {"message": "the amphood moved house",
...
async def broadcast(msg):
    ...
    The amphood does not roam because somebody is watching and does not stop when
...
def load_brain():
    if STATE["brain"] is None:
        ...
        fb = amphoodBrain()
        STATE["brain"] = fb
        STATE["pilot"] = amphoodPilot(fb, sim_steps=60)
        STATE["xy"] = soma_xy(fb)
...
# --------------------------------------------------------------------------
# what is under the cursor, and may the amphood click it
# --------------------------------------------------------------------------
UNDER_JS = """([x, y]) => {
...
async def roam(steps_per_page=44, headful=False, seed=None):
    ...
    async with async_playwright() as pw:
        ...
        # with nothing in it: the amphood cannot be logged in as anyone.
        ctx = await browser.new_context(
            ...
            user_agent=("Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                        ...
                        "Chrome/124.0 Safari/537.36 amphoodbrain/1.0"),
        )
        page = await ctx.new_page()
        page.on("dialog", lambda d: asyncio.create_task(d.dismiss()))

        # A popup is not somewhere the amphood chose to go, so it gets closed - but
        ...
        while STATE["running"]:
            ...
            # A amphood that walks off the bottom of what it can see should get
            ...
            # The amphood decides twice a second - that is 12 ms of brain time per
            ...
            # a amphood that has run out of page gets put back on a seed
...
def publish(stats, jpg, url, hz, neural=None):
    ...
    The site is static and the amphood runs on a machine, so something has to carry
    the state across. Writing it here keeps that transport separate from the
    roaming itself - if the publisher is not running, the amphood does not care.
    """
    try:
...
async def socket(ws: WebSocket):
    """A watcher. There is nothing to send: the amphood is already roaming."""
    await ws.accept()
    CLIENTS.add(ws)
    try:
        while True:
...
async def begin():
    ...
    """
    if load_env().get("amphood_ALLOW_BROWSER") != "1":
        say("amphood_ALLOW_BROWSER is not 1 - not opening a browser")
        return
...
if __name__ == "__main__":
    ...
    STATE["port"] = a.port
    load_brain()
    say(f"the amphood roams - open http://localhost:{a.port}")
    # loopback on a desk, every interface in a container
    uvicorn.run(app, host=os.environ.get("amphood_HOST", "127.0.0.1"),
