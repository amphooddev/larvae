...
MEASURED
  * How 77 amphood olfactory receptors respond to 690 odorants: DoOR 2.0 (the
  ...
  * The rate scale: amphood receptor neurons fire up to about 200 Hz (Hallem and
...
# a amphood can detect, so they are deliberately absent.
WORD_ODOURS = {
...
class Nose:
    ...
    `fb` needs `where(type_re=...)` returning neuron indices, as amphoodBrain does.
    """
    ...
    def drive(self, smell):
        """The smell as a amphoodBrain drive dict: receptor neurons of each glomerulus at response x max_hz."""
